...
// ---------- Matrix scan ----------
static inline void effective_cluster_and_taps(uint8_t cur_cluster, uint8_t raw_taps,
                                              uint8_t *out_cluster, uint8_t *out_taps) {
    uint8_t cluster = cur_cluster;
    uint8_t taps    = raw_taps;

    if (taps < 1) taps = 1;
    if (taps > 6) taps = 6;

    // 4..6 taps mean "other group, same position" and taps become 1..3
    if (taps > 3) {
        uint8_t grp = cluster_group(cluster);
        uint8_t pos = cluster_pos(cluster);
        cluster = ((1 - grp) * 3) + pos;
...
if (!hold_active && !hold_armed &&
    ...
    uint8_t eff_cluster = cur;
    uint8_t eff_taps    = pedal_taps;
    effective_cluster_and_taps(cur, pedal_taps, &eff_cluster, &eff_taps);

    // NEW: if raw taps were 4..6, persistently switch to the other group
    if (pedal_taps > 3) {
        set_cluster(eff_cluster);   // red LED + default layer now stay changed
    }

    run_cluster_macro(eff_cluster, eff_taps);
